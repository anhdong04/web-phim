module.exports = function applyV600(source) {
  // Web Phim v6.0:
  // 1) Lazy AI Vietnamese subtitle generation from English subtitles.
  // 2) Per-share Personal Home learned from actual stream requests.
  ...
  // ----------------------------------------------------------
  // AI subtitle signing, fetch, parsing, translation and cache.
  // ----------------------------------------------------------
  ...
  const helpers = [
    ...
    "    if (!r.ok) throw new Error('Subtitle HTTP ' + r.status); const len = Number(r.headers.get('content-length') || 0); if (len > V600_AI_SUB_MAX_BYTES) throw new Error('Subtitle quá lớn');",
    "    const text = await r.text(); if (Buffer.byteLength(text) > V600_AI_SUB_MAX_BYTES) throw new Error('Subtitle quá lớn'); return text;",
    ...
    "  return cues;",
    "}",
    "function v600DecodeHtml(s) { return String(s || '').replace(/&#(\\d+);/g, (_,n)=>String.fromCodePoint(Number(n))).replace(/&quot;/g,'\\"').replace(/&#39;|&apos;/g,\"'\").replace(/&lt;/g,'<').replace(/&gt;/g,'>').replace(/&amp;/g,'&'); }",
    ...
    "  const raw = await v600FetchSubtitleText(sourceUrl), cues = v600ParseCues(raw); if (!cues.length) throw new Error('Không đọc được subtitle');",
    ...
    "  const payload = v600AiDecodeToken(token); if (!payload) return sendJson(res, 404, { error: 'AI subtitle token invalid or translation not configured' }, 0);",
    "  try { return v600SendVtt(res, await v600TranslateSubtitle(payload.u)); } catch (e) { console.error('AI subtitle:', e.message); return sendJson(res, 502, { error: 'Không tạo được phụ đề AI' }, 0); }",
    "}",
    "",
    // Personal Home storage/profile helpers.
    ...
    "  if (id === 'personal-because') {",
    "    const recent = v600RecentOfType(profile, type); if (recent?.tmdbId) { try { const p = await fetchJson(tmdbUrl((type === 'movie' ? '/movie/' : '/tv/') + recent.tmdbId + '/recommendations', { page }), 'TMDB personal recommendations'); results = p.results || []; } catch {} }",
    "  }",
    "  if (!results.length) {",
  ...
  // ----------------------------------------------------------
  // Add Personal Home rows to the manifest.
  // ----------------------------------------------------------
  ...
  // Personal catalog routing gets the share id so each short link learns independently.
  ...
  // ----------------------------------------------------------
  // AI Vietnamese subtitles are added only when a ranked VI track is absent.
  // Existing real VI subtitles always win. KKPhim remains untouched.
  // ----------------------------------------------------------
  ...
  source = source.replace(attachMarker, aiWrapper);
  ...
  // Serve signed, lazy-translated WebVTT tracks under both root and share/config bases.
