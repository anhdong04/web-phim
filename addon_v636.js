// Web Phim v6.3.6 launcher.
// Builds on v6.3.5 and adds strict Dailymotion title matching for YanHH3D fallback.
...
if (!launcher.includes(finalEval)) {
  console.error('v6.3.6 patch target missing: v4.1 final eval');
  process.exit(1);
}
const runV636 = [
...
launcher = launcher.replace(finalEval, runV636);
eval(launcher);
