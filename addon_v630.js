// Web Phim v6.3.0 launcher.
// Builds on v6.2.9 and aligns YanHH3D stream/video identity with Nuvio Desktop.
...
if (!launcher.includes(finalEval)) {
  console.error('v6.3.0 patch target missing: v4.1 final eval');
  process.exit(1);
}
const runV630 = [
...
launcher = launcher.replace(finalEval, runV630);
eval(launcher);
