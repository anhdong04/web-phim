// Web Phim v6.2.5 launcher.
// Builds on v6.2.4 and adds YanHH3D catalog + stream provider.
...
if (!launcher.includes(finalEval)) {
  console.error('v6.2.5 patch target missing: v4.1 final eval');
  process.exit(1);
}
const runV625 = [
...
launcher = launcher.replace(finalEval, runV625);
eval(launcher);
