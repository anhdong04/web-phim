// Web Phim v6.3.4 launcher.
// Builds on v6.3.3 and prefers MPV-compatible YanHH3D servers with Dailymotion fallback.
...
if (!launcher.includes(finalEval)) {
  console.error('v6.3.4 patch target missing: v4.1 final eval');
  process.exit(1);
}
const runV634 = [
...
launcher = launcher.replace(finalEval, runV634);
eval(launcher);
