// Web Phim v6.2.8 launcher.
// Builds on v6.2.7 and adds browser-safe YanHH3D HLS relay/CORS handling.
...
if (!launcher.includes(finalEval)) {
  console.error('v6.2.8 patch target missing: v4.1 final eval');
  process.exit(1);
}
const runV628 = [
...
launcher = launcher.replace(finalEval, runV628);
eval(launcher);
