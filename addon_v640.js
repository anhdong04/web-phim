// Web Phim v6.4.0 launcher.
// Builds on v6.3.9 and adds a Windows VLC external playback fallback for YanHH3D.
...
if (!launcher.includes(finalEval)) {
  console.error('v6.4.0 patch target missing: v4.1 final eval');
  process.exit(1);
}
const runV640 = [
...
launcher = launcher.replace(finalEval, runV640);
eval(launcher);
