module.exports = function applyV420(source) {
  const defaultsMarker = "  subtitlePreferNonHI: String(process.env.SUBTITLE_PREFER_NON_HI || 'true').toLowerCase() !== 'false'";
  if (!source.includes(defaultsMarker)) throw new Error('v4.2 patch target missing: DEFAULTS');
  ...
  if (!source.includes(sanitizeMarker)) throw new Error('v4.2 patch target missing: sanitizeConfig');
  ...
  if (!source.includes(fetchMarker)) throw new Error('v4.2 patch target missing: fetchJson marker');
  const timeoutHelper = [
    "function v420ProviderTimeout(provider) {",
  ...
  source = source.replace(fetchMarker, timeoutHelper);

  const requestTimer = "  const timer = setTimeout(() => controller.abort(), REQUEST_TIMEOUT_MS);";
  if (!source.includes(requestTimer)) throw new Error('v4.2 patch target missing: fetch timeout');
  ...
  if (!scorerRe.test(source)) throw new Error('v4.2 patch target missing: streamScore');
  const scorer = [
    "function v420Codec(s) {",
    ...
    "      score += audio === 'premium' ? 1800 : 0; break;",
    "    case '1080p-quality':",
    "      score += r === 1080 ? 9000 : r === 2160 ? 1000 : r === 720 ? 2500 : 0;",
    "      score -= gb > 35 ? (gb - 35) * 120 : 0; break;",
    "    case '1080p-balanced':",
    "      score += r === 1080 ? 9500 : r === 720 ? 3500 : r === 2160 ? -1500 : 0;",
    "      score -= gb > 18 ? (gb - 18) * 220 : 0; break;",
    "    case 'mobile':",
    "      score += r === 1080 ? 6500 : r === 720 ? 8500 : r === 480 ? 4500 : r === 2160 ? -7000 : 0;",
    "      score -= gb * 450; score += audio === 'small' ? 900 : 0; break;",
    "    case 'low-bandwidth':",
    "      score += r === 720 ? 9000 : r === 480 ? 8000 : r === 1080 ? 2000 : r === 2160 ? -9000 : 0;",
    "      score -= gb * 800; break;",
    "  }",
    "  if (cfg.preferredCodec !== 'auto') score += codec === cfg.preferredCodec ? 1800 : codec === 'unknown' ? 0 : -900;",
    "  else if ((cfg.deviceProfile === 'mobile' || cfg.deviceProfile === 'low-bandwidth') && (codec === 'hevc' || codec === 'av1')) score += 500;",
    ...
    "  else { const rp = r === 1080 ? 20000 : r === 2160 ? 18000 : r === 1440 ? 16000 : r === 720 ? 12000 : 7000; const penalty = gb > 25 ? (gb - 25) * 180 : 0; score = rp + (hdr ? 1800 : 0) + (premiumSource ? 1400 : 0) + (premiumAudio ? 700 : 0) - penalty; }",
    "  if (cfg.preferCached && cached) score += 6000;",
    "  score += v420ProfileScore(s, cfg);",
    "  return score;",
    "}"
  ].join('\n');
  source = source.replace(scorerRe, scorer);
  ...
  const statusOld = "configureSupported: true, healthSupported: true, tmdbMemoryCache: true, circuitBreaker: true, configuredManifest: configured, streamPreset:";
  if (!source.includes(statusOld)) throw new Error('v4.2 patch target missing: status payload');
  source = source.replace(statusOld, "configureSupported: true, configureVersion: 2, scoringVersion: 2, providerSpecificTimeouts: true, healthSupported: true, tmdbMemoryCache: true, circuitBreaker: true, configuredManifest: configured, deviceProfile: cfg.deviceProfile, preferredCodec: cfg.preferredCodec, preferredAudio: cfg.preferredAudio, streamPreset:");
