...
const manifest = {
  ...
  description: 'Vietnamese TMDB home catalogs with KKPhim + Comet/TorBox streams',
  resources: [
    'catalog',
  ...
  types: ['movie', 'series'],
  catalogs: [
    { type: 'movie', id: 'home-hot', name: '🔥 Hot rần rần', extra: catalogExtra },
    { type: 'movie', id: 'home-top10', name: '🏆 Top 10 hôm nay', extra: catalogExtra },
    { type: 'movie', id: 'home-new-movies', name: '🆕 Phim lẻ mới cập nhật', extra: catalogExtra },
    { type: 'series', id: 'home-new-series', name: '📺 Phim bộ mới nhất', extra: catalogExtra },
    { type: 'movie', id: 'home-animation', name: '🧸 Hoạt hình', extra: catalogExtra },
    { type: 'series', id: 'home-korean', name: '🇰🇷 Phim Hàn Quốc', extra: catalogExtra },
    { type: 'movie', id: 'home-horror', name: '👻 Kinh dị', extra: catalogExtra },
    { type: 'movie', id: 'home-action', name: '⚔️ Hành động', extra: catalogExtra }
  ]
};
...
async function fetchJson(url, label = 'upstream') {
  ...
  try {
    const r = await fetch(url, {
      headers: { accept: 'application/json', 'user-agent': 'web-phim-v3/3.1' },
...
function genres(list) {
  return Array.isArray(list) ? list.map(x => x.name).filter(Boolean) : [];
}
function castNames(credits) {
  return (credits?.cast || []).slice(0, 12).map(x => x.name).filter(Boolean);
}
function directorNames(credits) {
  return (credits?.crew || []).filter(x => x.job === 'Director').slice(0, 5).map(x => x.name).filter(Boolean);
}
...
async function getMeta(type, id) {
  ...
  if (type === 'movie') {
    const p = await fetchJson(
      tmdbUrl(`/movie/${tmdbId}`, { append_to_response: 'external_ids,credits' }),
      'TMDB movie'
    );
    return {
      id,
  ...
  const p = await fetchJson(
    tmdbUrl(`/tv/${tmdbId}`, { append_to_response: 'external_ids,credits' }),
    'TMDB tv'
  );
...
async function resolveImdb(type, parsed) {
  ...
  const path = type === 'movie'
    ? `/movie/${parsed.tmdbId}/external_ids`
    : `/tv/${parsed.tmdbId}/external_ids`;
...
function inferEpisode(item) {
  for (const v of [item?.name, item?.slug, item?.filename]) {
    ...
    m = t.match(/(?:tap|episode|ep|e)\s*[-_. ]*0*(\d+)\b/);
    if (m) return Number(m[1]);
  }
  return null;
}
...
async function getKKPhim(type, parsed) {
  let url;
  if (parsed.kind === 'tmdb') {
    url = `${KKPHIM_API}/tmdb/${type === 'series' ? 'tv' : 'movie'}/${parsed.tmdbId}`;
  } else {
    url = `${KKPHIM_API}/imdb/title/${parsed.imdbId}`;
  }
...
function configuredUpstreams() {
  ...
  for (const [name, key] of defs) {
    if (process.env[key]) out.push({ name, url: process.env[key].trim() });
  }
...
async function getUpstream(source, type, id) {
  try {
    ...
    return (p.streams || []).map(s => ({
      ...s,
      name: source.name === 'Comet'
        ? '⚡ Comet / TorBox'
        : `${source.name}${s.name ? ` • ${s.name}` : ''}`
    }));
  } catch {
    return [];
  }
...
function streamKey(s) {
  return s.url || s.externalUrl || (s.infoHash ? `${s.infoHash}:${s.fileIdx ?? ''}` : JSON.stringify(s));
}
...
function curate(streams) {
  ...
  if (MAX_SIZE_GB > 0) {
    a = a.filter(s =>
      !sizeBytes(s) ||
      sizeBytes(s) <= MAX_SIZE_GB * 1024 ** 3 ||
      String(s.name || '').includes('KKPhim')
    );
  }
...
const server = http.createServer(async (req, res) => {
  ...
  if (u.pathname === '/') {
    return sendJson(res, 200, {
      ...
      homeLayout: manifest.catalogs.map(c => c.name)
    }, 60);
  }
  ...
  if (m) {
    try {
      const extra = m[3] ? JSON.parse(decodeURIComponent(m[3])) : {};
      return sendJson(res, 200, { metas: await getCatalog(m[1], m[2], extra.skip) }, 900);
    } catch (e) {
      console.error(e);
  ...
  if (m) {
    try {
      return sendJson(res, 200, { meta: await getMeta(m[1], decodeURIComponent(m[2])) }, 900);
    } catch (e) {
      console.error(e);
  ...
  if (m) {
    try {
      return sendJson(res, 200, { streams: await getStreams(m[1], decodeURIComponent(m[2])) }, 180);
    } catch (e) {
      console.error(e);
...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`Phim Việt + TorBox v3.1 listening on ${PORT}`);
});
