// Web Phim v6.4.2 launcher.
// Builds on v6.4.1 and adds the isolated /vn source bridge.
...
const applyV652IptvForceRoute = require('./v652_iptv_force_route_patch');

let launcher = fs.readFileSync(require.resolve('./addon_v410.js'), 'utf8');
const finalEval = 'eval(source);';
if (!launcher.includes(finalEval)) {
  console.error('v6.4.2 patch target missing: v4.1 final eval');
  process.exit(1);
}
const runV642 = [
  ...
  'source = applyV652IptvForceRoute(source);',
...
launcher = launcher.replace(finalEval, runV642);
eval(launcher);