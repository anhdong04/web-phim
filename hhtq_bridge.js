...
function hostOf(value){try{return new URL(String(value||'')).host||null;}catch{return null;}}
function summarizeLinks(rows){return (rows||[]).slice(0,12).map(x=>({serverName:x?.serverName||null,hasUrl:!!x?.url,urlHost:hostOf(x?.url),isM3u8:!!x?.isM3u8,hasExternalUrl:!!x?.externalUrl,externalHost:hostOf(x?.externalUrl||x?.embedUrl)}));}
...
function manifest() {
  return {
    ...
    resources:['catalog',{name:'meta',types:['movie','series'],idPrefixes:['hhtq:']},{name:'stream',types:['movie','series'],idPrefixes:['hhtq:']}],
...
async function metaFor(type,id) {
  ...
  const videos=eps.map((ep,index)=>({id:`hhtq:${parsed.detailKey}:e:${index}`,title:ep.name || `Tập ${index+1}`,season:1,episode:Number.isFinite(ep.number)?ep.number:index+1}));
  const effectiveType=type==='movie'?'movie':'series';
  const meta={id:`hhtq:${parsed.detailKey}`,type:effectiveType,name:d.title,poster:d.posterUrl||undefined,background:d.background||d.posterUrl||undefined,description:d.description||'Hoạt hình Trung Quốc',releaseInfo:d.year?String(d.year):undefined,genres:d.genres||['Hoạt hình Trung Quốc']};
...
function streamObject(link,title) {
  const s={name:'🐲 HHTQ',title:[link.serverName,title].filter(Boolean).join(' • '),behaviorHints:{notWebReady:!link.isM3u8,bingeGroup:'webphim-hhtq-v1'}};
  if(link.url){s.url=link.url;if(link.headers&&Object.keys(link.headers).length)s.behaviorHints.proxyHeaders={request:link.headers};}
  return s;
}
async function streamsFor(type,id) {
  ...
  if(!ep?.watchUrl && type==='movie') ep={name:'FULL',number:1,watchUrl:parsed.detailUrl,serverName:'HHTQ'};
...
async function diag() {
  ...
  return {ok:items.length>0,...buildInfo(),baseUrl:provider.mainUrl,elapsedMs:Date.now()-started,itemCount:items.length,firstTitle:items[0]?.title||null,firstEpisodes:detail?.episodes?.length||0};
}
async function diagnoseMovie185(){
  const started=Date.now();
  const detailUrl=`${String(provider.mainUrl||'https://hhhtq.team').replace(/\/+$/,'')}/phim/185/`;
  const out={ok:false,diagnostic:'hhtq-movie-185-inline-v2',...buildInfo(),detailUrl,baseUrl:provider.mainUrl,steps:{}};
  let html='';
  try{html=await provider.fetchText(detailUrl,provider.mainUrl);const urls=playerAaaaUrls(html,detailUrl);out.steps.fetchDetailHtml={ok:true,bytes:Buffer.byteLength(html),playerAaaaCount:urls.length,playerHosts:urls.map(hostOf)};}catch(e){out.steps.fetchDetailHtml={ok:false,error:String(e?.message||e).slice(0,300)};}
  let detail=null;
  try{detail=await provider.detail(detailUrl);out.steps.detail={ok:true,title:detail?.title||null,type:detail?.type||null,episodeCount:Array.isArray(detail?.episodes)?detail.episodes.length:0,episodes:(detail?.episodes||[]).slice(0,6).map(ep=>({name:ep?.name||null,number:ep?.number??null,watchUrl:ep?.watchUrl||null}))};}catch(e){out.steps.detail={ok:false,error:String(e?.message||e).slice(0,300)};}
  const ep=detail?.episodes?.[0]||{name:'FULL',number:1,watchUrl:detailUrl};
  let exact=[];try{exact=await provider.streams(ep.watchUrl||detailUrl);out.steps.exactResolver={ok:true,count:exact.length,links:summarizeLinks(exact)};}catch(e){out.steps.exactResolver={ok:false,error:String(e?.message||e).slice(0,300)};}
  let fallback=[];try{fallback=await resolveFallback(detail?.title||'Tây Hành Kỷ Ám Ảnh Ma Thành',ep);out.steps.hh4kFallback={ok:true,count:fallback.length,links:summarizeLinks(fallback)};}catch(e){out.steps.hh4kFallback={ok:false,error:String(e?.message||e).slice(0,300)};}
  out.ok=exact.some(x=>x?.url)||fallback.some(x=>x?.url);out.finalDirectCount=[...exact,...fallback].filter(x=>x?.url).length;out.elapsedMs=Date.now()-started;return out;
}
...
async function handleHhtq(req,res,pathname) {
  ...
  if(pathname==='/hhtq'||pathname==='/hhtq/') return sendJson(req,res,200,{addon:'HHTQ',manifest:'/hhtq/manifest.json',...buildInfo()});
  if(pathname==='/hhtq/build') return sendJson(req,res,200,buildInfo());
  if(pathname==='/hhtq/manifest.json') return sendJson(req,res,200,manifest(),30);
  if(pathname==='/hhtq/diag'){try{return sendJson(req,res,200,await diag());}catch(e){return sendJson(req,res,200,{ok:false,...buildInfo(),error:String(e?.message||e).slice(0,400)});}}
  if(pathname==='/hhtq/diag/movie-185'){try{return sendJson(req,res,200,await diagnoseMovie185());}catch(e){return sendJson(req,res,200,{ok:false,...buildInfo(),diagnostic:'hhtq-movie-185-inline-v2',error:String(e?.message||e).slice(0,400)});}}
  ...
  return sendJson(req,res,404,{error:'HHTQ route not found',...buildInfo()});
}
...
http.createServer=function patchedCreateServer(...args){
  ...
  args[0]=function wrapped(req,res){let pathname='/';try{pathname=new URL(req.url,`http://${req.headers.host||'localhost'}`).pathname;}catch{}if(pathname==='/hhtq'||pathname.startsWith('/hhtq/')){Promise.resolve(handleHhtq(req,res,pathname)).catch(err=>{console.error('[hhtq] unhandled:',err);if(!res.headersSent)sendJson(req,res,500,{error:'HHTQ internal error',...buildInfo()});else res.end();});return;}return downstream(req,res);};
  return originalCreateServer.apply(http,args);
};
console.log('[hhtq] bridge v1.0.2 enabled at /hhtq/*');
module.exports={manifest,catalog,metaFor,streamsFor,parseId,rootId,handleHhtq,CATALOGS,CATEGORY_PATHS,diagnoseMovie185,buildInfo};
