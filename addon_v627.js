// Web Phim v6.2.7 launcher.
// Builds on v6.2.6 and makes YanHH3D relay explicitly HLS-aware for Nuvio.
...
if (!launcher.includes(finalEval)) {
  console.error('v6.2.7 patch target missing: v4.1 final eval');
  process.exit(1);
}
const runV627 = [
...
launcher = launcher.replace(finalEval, runV627);
eval(launcher);
