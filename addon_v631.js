// Web Phim v6.3.1 launcher.
// Builds on v6.3.0 and makes the YanHH3D relay safe for Nuvio Desktop/libmpv HTTP probing.
...
if (!launcher.includes(finalEval)) {
  console.error('v6.3.1 patch target missing: v4.1 final eval');
  process.exit(1);
}
const runV631 = [
...
launcher = launcher.replace(finalEval, runV631);
eval(launcher);
