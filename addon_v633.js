// Web Phim v6.3.3 launcher.
// Builds on v6.3.2 and rejects YanHH3D PNG-carrier HLS in favor of plain media streams.
...
if (!launcher.includes(finalEval)) {
  console.error('v6.3.3 patch target missing: v4.1 final eval');
  process.exit(1);
}
const runV633 = [
...
launcher = launcher.replace(finalEval, runV633);
eval(launcher);
